/* ============================================================
   ...
   Horizontal paged navigation, every device. No dependencies.
   ============================================================ */
...
(function () {
  ...
  // Matches the mobile block in styles.css. Used only to pick an image
  // tier and to gate the desktop right-click menu — never for layout.
  const phoneMQ = window.matchMedia('(max-width: 899px)');
  ...
  // panel animates in.
  //
  // loading="lazy" is not an option anywhere in this file: a horizontally
  // scrolled panel never fires the intersection the browser waits on, so
  // the frames simply stay black.
  ...
  // See images/README.txt — every photograph exists in three tiers.
  ...
  function lbShow(i) {
    ...
    // A phone was pulling the 2200px original — roughly 1 MB a frame, for
    // a screen that cannot show a quarter of it. The 1400px tier is about
    // 115 KB. Falls back to the original if a medium file is ever missing.
    if (isPhone()) {
  ...
  // ── Tap versus flick ─────────────────────────────────────────
  // A swipe that starts on a thumbnail would otherwise be read as a tap
  // and throw the lightbox open mid-gesture. A mouse click displaces
  // nothing, so this costs desktop nothing and needs no device check.
  // pointerup lands before click, so the flag is current when read.
  ...
  document.addEventListener('contextmenu', function (e) {
    ...
    // stealing the phone's own image menu.
